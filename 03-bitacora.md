...
| Tabla de verificación (`GET /api/patients`, consulta combinada del Anexo A) | `api/src/TbtbPatientTracking.Application/Patients/PatientService.cs` (`ListAsync`), `PatientListItemResponse.cs`, `api/src/TbtbPatientTracking.Api/Controllers/PatientsController.cs` | `api/tests/TbtbPatientTracking.Tests/Patients/PatientListValidationTests.cs` (parámetros de paginación) y verificación manual contra SQL Server con `scripts/002_datos_prueba.sql` (la consulta no corre en SQLite; ver registro de decisiones) | Parcial (API cubierta; falta la interfaz, Fase 5) |
...
- 2026-09-23 · **Rechazo mío sobre la propuesta del asistente (pacientes inactivos):** el asistente propuso permitir registrar contactos de un paciente inactivo (`IsActive = false`), con el rechazo con `400` como alternativa. Lo rechacé: no se puede registrar un contacto de un paciente inactivo y el servicio responde `400` · Si el paciente está inactivo, queda a criterio del gestor volver a llamarle para saber qué pasó con él, fuera de este registro · corrección mía sobre la propuesta del asistente; una prueba documenta la regla.
...
- 2026-09-23 · El listado (`GET /api/patients`) ordena del paciente más reciente al más antiguo (`CreatedAt DESC`, con `Id` como desempate para que la paginación sea estable) y usa un tamaño de página por defecto de 25 (máximo 100) · Ayuda a verificar el CA-1, porque el paciente recién registrado aparece arriba · el asistente propuso el orden y un tamaño de 20; yo aprobé el orden y cambié el tamaño a 25.
- 2026-09-23 · El listado muestra el número de documento completo, sin enmascarar · Es una herramienta interna del gestor y lo necesita para distinguir pacientes; enmascararlo queda como mejora si se expone a más roles · el asistente propuso dejarlo completo; quedó así sin objeción mía por ahora.
- 2026-09-23 · **Rechazo mío sobre la propuesta del asistente (pruebas de la consulta del listado):** SQLite no puede ordenar ni comparar `DateTimeOffset`, y la consulta ordena por `CreatedAt` y `ContactDate`. El asistente propuso un contexto de pruebas con un conversor a ticks UTC para poder ejecutar la consulta real en SQLite. Lo rechacé: es solo una lista y no vale el tiempo ni la diferencia entre el mapeo de pruebas y el de producción. Consecuencia: la consulta no tiene prueba automatizada; las pruebas cubren los parámetros de paginación y la consulta se verifica a mano contra SQL Server con los datos de `scripts/002_datos_prueba.sql` (total y último contacto por paciente, y que un contacto anulado no cuenta). Es una limitación conocida · corrección mía sobre la propuesta del asistente.
- 2026-09-23 · **Justificación de la consulta combinada (Anexo A) y su índice:** el listado es una sola consulta con proyección. Se pagina primero `Patients` (`OFFSET/FETCH`) y, solo para las filas de esa página, se ejecutan dos subconsultas correlacionadas por `PatientId` sobre `Contacts`: el total (`COUNT` con `IsDeleted = 0`) y el último contacto (`TOP 1` por `ContactDate DESC`, también sin anulados). Alternativas descartadas: (1) traer los pacientes y luego los contactos de cada uno en un ciclo (N+1 consultas); (2) traer todos los contactos y agrupar en memoria (crece con el historial); (3) un `GROUP BY` o `ROW_NUMBER` sobre toda la tabla `Contacts` unida a `Patients` (recorre todos los contactos aunque se muestren 25 pacientes). **Índice:** `IX_Contacts_PatientId_ContactDate` sobre `(PatientId, ContactDate DESC)` filtrado por `IsDeleted = 0`: cada subconsulta entra directo a los contactos del paciente, ya ordenados por fecha, sin ordenar y sin leer los anulados. Con unos 400 pacientes de arranque basta ese índice. Con volumen real (miles de pacientes y cientos de miles de contactos) los siguientes pasos serían: (a) agregar `INCLUDE (Channel, ResultCode, GestorUsername)` al índice para leer el último contacto sin volver a la tabla; (b) un índice en `Patients (CreatedAt DESC, Id)` para que el orden y la paginación no ordenen toda la tabla en cada página. Ambos requieren una migración nueva y no se incluyen ahora porque con 400 filas no aportan. No se midió el plan de ejecución en esta entrega; el uso del índice se puede confirmar con el plan real · asistente propuso, yo aprobé.
...
- Rechazos o correcciones: rechacé la propuesta del asistente de permitir contactos de pacientes inactivos; el servicio los rechaza con `400` y hay una prueba que lo documenta (ver registro de decisiones).

### Commit 6: Fase 4: GET /api/patients con consulta combinada (paginación, total de contactos y último contacto)
- Hash: (se completa al inicio de la siguiente fase)
- Fase: 4
- Qué cambió y por qué: `GET /api/patients?page=1&pageSize=25` devuelve una página de pacientes (más recientes primero) con el total de contactos vigentes y el último contacto de cada uno, excluyendo contactos anulados. Es la tabla de verificación del CA-1 y CA-2 y la consulta combinada del Anexo A. Parámetros inválidos responden `400` por campo. La justificación de la consulta y del índice está en el registro de decisiones.
- Archivos principales: `api/src/TbtbPatientTracking.Application/Patients/PatientService.cs`, `IPatientService.cs`, `PatientListItemResponse.cs`, `api/src/TbtbPatientTracking.Application/Common/PagedResponse.cs`, `Pagination.cs`, `api/src/TbtbPatientTracking.Api/Controllers/PatientsController.cs`, `api/tests/TbtbPatientTracking.Tests/Patients/PatientListValidationTests.cs`
- Criterio relacionado: tabla de verificación de CA-1 y CA-2 (consulta combinada del Anexo A)
- Prueba que lo verifica: `PatientListValidationTests` (parámetros de paginación). Resultado: pendiente de ejecutar. La consulta se verifica a mano contra SQL Server (resultado: pendiente).
- Propuesta de: asistente, con decisiones mías sobre orden, tamaño de página y sobre no probar la consulta en SQLite
- Rechazos o correcciones: rechacé el conversor de fechas para probar la consulta en SQLite (ver registro de decisiones). Se corrigió además la bitácora de la Fase 3, que seguía diciendo que se permitían contactos de pacientes inactivos.
